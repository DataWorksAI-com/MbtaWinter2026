...
_switchboard_cache_ttl = timedelta(seconds=0)
...
async def get_switchboard_candidates(capabilities: Optional[List[str]] = None) -> List[Dict]:
    ...
    url = f"{REGISTRY_URL}/switchboard/discover?q=mbta"

    try:
        raw = await _aio.to_thread(
            lambda: _json.loads(_ur.urlopen(url, timeout=60).read())
        )
        candidates = raw.get("agents", [])
        registries = list(set(a.get("source", "") for a in candidates))
        logger.info(f"✓ Switchboard: {len(candidates)} candidates from {registries}")

        if candidates:  # Only cache non-empty results
...
async def semantic_discovery(query: str) -> List[AgentConfig]:
    ...
    if USE_SWITCHBOARD:
        ...
        MBTA_AGENT_IDS = {"mbta-alerts", "mbta-stopfinder", "skill-mbta-stopfinder", "mbta-planner"}
        candidates = [c for c in candidates if 
           any(x in c.get("agent_id", "").lower() for x in ["mbta-alerts", "mbta-stopfinder", "mbta-planner"])]
